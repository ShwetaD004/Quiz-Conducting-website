...
const registrationData = () =>{
    if(localStorage.getItem(allInput[0].value+"_brand") == null){
        ...
        registerForm.reset('');
        swal("Registration Done!", "Next step do Sign In!", "success");
    }else{
        swal(
          "Please Change Brand Code!",
          "This brand code already taken !",
          "warning"
        );
    }
}
...
signinBtn.onclick = function(e){
    ...
    if(brandCode.value && username.value && password.value != ""){
        if(localStorage.getItem(brandCode.value+"_brand") != null){
            ...
            swal("Wrong Brand Code !", "Please signup first ! ", "warning");
        }
    }else{
        swal(
          "Empty field !",
