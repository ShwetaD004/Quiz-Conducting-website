...
const newSubject = (subject, index) => {
  ...
  for (i = 0; i < delAllBtn.length; i++) {
    delAllBtn[i].onclick = function () {
      ...
      swal({
        title: "Are you sure?",
        text: "Once deleted, you will not be able to recover this subject file!",
        icon: "warning",
        buttons: true,
        dangerMode: true,
      }).then((willDelete) => {
        if (willDelete) {
          parent.remove();
          updateSubject();
          swal("Poof! Your Subject has been deleted!", {
            icon: "success",
          });
        } else {
...
// start return subject in create question form
...
function insertQuestionFun(sub,id,question,opOne,opTwo,opThree,opFour,corAns) {
  if(sub != undefined && id != undefined){
    ...
    if (chooseSubject.value != "choose subject") {
      ...
      swal("Success ", "Question created successfully", "success");
      questionForm.reset("");
    } else {
            ...
            <div class="mb-5" index = "${index}">
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <div>
                        ...
                        <br><br>
                        <span class="bg-info text-white p-3">${
    ...
    allEditBtn[i].onclick = function(){
      ...
      saveBtn.onclick = function(){
        ...
        var corAns = span[4].innerHTML;
              swal({
                title: "Are you sure?",
                text: "Once updated, you will not be able to recover this subject file!",
                icon: "warning",
                buttons: true,
                dangerMode: true,
              }).then((willUpdated) => {
                if (willUpdated) {
                  insertQuestionFun(subject,index,question,opOne,opTwo,opThree,opFour,corAns);
...
if(localStorage.getItem(brandCode+"_registrationData") != null){
  registrationData = JSON.parse(localStorage.getItem(
...
const registrationFunc = () =>{
  if(userType.value != "choose type"){
    ...
    swal("Choose Type ", "Please select user type", "warning");
  }
}
    ...
    allEditBtn[i].onclick = function(){
      ...
      modalEditBtn.onclick = function(){
        ...
        modalUpdateBtn.onclick = function(){
          ...
          swal({
            title: "Are you sure?",
            text: "Once updated, you will not be able to recover this previos data!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
          }).then((willUpdated) => {
            if (willUpdated) {
              ...
              closeBtn.click();
              swal("Poof! Your profile has been updated!", {
                icon: "success",
              });
            } else {
              swal("Your imaginary file is safe!");
